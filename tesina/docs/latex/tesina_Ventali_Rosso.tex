...
create table UTENTI(
	...
	unique (username)
				...
				\subsubsection{Transazioni}
					\textbf{Esempio}\\ 
					\lstinputlisting[language=XML]{../../xml/transazioni.xml}
					\bigskip \textbf{Grammatica}\\
					\lstinputlisting[language=XML, backgroundcolor=\color{coloreSfondoGrammatiche}]{../../xml/schemaTransazioni.xsd}
		...
		\subsection{Analisi di alcuni elementi/attributi} \hypersetup{linkcolor=blue}
			...
			\subsubsection{Catalogo prodotti} 
				\begin{itemize}
					...
					\item \textbf{catalogo $\rightarrow$ prodotto $\rightarrow$ offerta\_speciale[crediti] } l'attributo crediti dell'elemento offerta\_speciale permette di definire la ricompensa di crediti bonus erogata al cliente che acquista il prodotto durante l'offerta speciale (\ref{OffSpec});
				\end{itemize}
			\smallskip
			...
			\subsubsection{Transazioni} 
			\begin{itemize}
				\item \textbf{transazioni $\rightarrow$ transazione[id\_cliente] } l'attributo id\_cliente dell'elemento transazione permette di specificare il cliente che ha effettuato la transazione (\ref{Introduzione});
				\item \textbf{transazioni $\rightarrow$ transazione[crediti\_bonus\_ricevuti] } l'attributo crediti\_bonus\_ricevuti dell'elemento transazione permette di specificare l'ammontare di crediti bonus ricevuti dal cliente dopo la transazione (\ref{PortBonus});
				\item \textbf{transazioni $\rightarrow$ transazione $\rightarrow$ prodotti } l'elemento prodotti dell'elemento transazione permette di specificare la lista di prodotti (con almeno un prodotto) coinvolti nella transazione (\ref{Introduzione});
				\item \textbf{transazioni $\rightarrow$ transazione $\rightarrow$ indirizzo\_consegna } l'elemento indirizzo\_consegna dell'elemento transazione permette di specificare l'indirizzo di consegna relativo alla transazione (sezione Clienti di \ref{CategorieUtenti});
			\end{itemize}
			\smallskip
			\subsubsection{Portafogli bonus} 
			\begin{itemize}
				...
				\item \textbf{portafogli $\rightarrow$ portafoglio $\rightarrow$ saldo } l'elemento saldo dell'elemento portafoglio permette di specificare la l'ammontare di crediti bonus posseduti dal cliente riferiti ad un determinato anno di acquisizione (sezione Sconto variabile di \ref{Sconti});
